...
local plrInfo = game.ReplicatedStorage["Player Info"][driver.Name]
...
seat.Changed:Connect(function(property)
	if (property == "Occupant") then
		...
		if (seat.Occupant ~= nil) then		
			local folder = script.Parent.DriverControls:Clone()
			folder.Parent = driver.PlayerGui

			folder.Vehicle.Value = model
			folder.Vehicle.Parent = driver
...
if (driver.Character == nil) then driver.CharacterAdded:Wait() end
repeat wait() until driver.Character:FindFirstChild("Humanoid")
driver.Character.Humanoid.JumpPower = 0
